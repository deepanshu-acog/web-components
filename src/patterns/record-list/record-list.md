...
kind: pattern
group: data-display
summary: A list of records, each with a name, a status, secondary detail, and a category.
uses: [wa-badge]
use: >
  Showing a set of similar records where each one has a name, some kind of
  status or severity, and a short supporting detail. Medicines, problems,
  alerts, tasks, findings, activity. Use it whenever the alternative would be a
  table with two or three columns.
avoid: >
  Do not use it when the records have four or more attributes worth comparing
  across rows, or when the user needs to sort or filter — that is a table, not
  a list. Do not use it for navigation menus (use wa-dropdown) or for a list of
  links (use plain markup).
---
